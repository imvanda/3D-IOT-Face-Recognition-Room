...
```bash
# 启动所有后端服务 (Node-RED, ThingsBoard, Mosquitto)
docker-compose up -d
...
访问 [http://localhost:1880](http://localhost:1880) 导入流程配置：
...
1. 在 `store.ts` 中的 `devices` 数组添加设备定义
2. 在 `components/Room.tsx` 中添加设备3D模型
...
- [DeepFace](https://github.com/serengil/deepface) - 人脸识别
